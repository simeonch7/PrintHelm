...
Private Sub Class_Globals
	...
	Private btnRetry, btnAbort, btnIgnore, btnZReport, btnXReport As Button
...
Public Sub Build(parent As MiScrollView,ParentWidth As Int, ParentHeight As Int)
	...
	btnIgnore.Text =Main.translate.GetString("strIgnore")
	btnIgnore.Tag = PrinterConstants.Action_Ignore
	HelperFunctions.setButtonStyle(btnIgnore)
	HelperFunctions.Remove_Padding(btnIgnore)
	...
	Dim btnNormal As Int = (foot.Width- 2*padding)/3
	...
	footNormal.AddView(btnRetry,0,0,btnNormal,foot.Height)
	footNormal.AddView(btnAbort,btnNormal+padding,0,btnNormal,foot.Height)
	footNormal.AddView(btnIgnore,2*btnNormal+2*padding,0,btnNormal,foot.Height)
...
'Example:
'dim userAction as ResumableSub = statusItem.getUserAction(PrinterConstants.ERR_NoCommunication)
...
Public Sub getUserAction(error As Int) As ResumableSub
	...
	Wait For (Action_Click) Complete (Result As Int)
